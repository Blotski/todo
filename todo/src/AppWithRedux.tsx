// this must be with Redux
// issues uncheck all other items when using DELETE
...
function AppWithRedux(): React.ReactElement { 
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [ loggedIn, setLoggedIn ] = useState(true);
  const [ newItem, setNewItem ] = useState<TodoItem >('');
  // const [ todoItems, setTodoItems ] = useState<TodoItem[]>([])
  // const [ filter, setFilter ] = useState<filter>('all') // add to RTK first

  const dispatch: Dispatch = useDispatch();
  const filter: filter = useSelector((state: RootState) => state.filter.value)
  const todoItmesR: TodoItem[] = useSelector((state: RootState ) => state.todoItems.value)
